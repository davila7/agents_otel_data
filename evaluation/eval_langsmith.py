...
def post(path, body, **kw):
    return _req("POST", path, json=body, **kw)


# ---------- 1. auth + project uuid ----------
r = get("/sessions", name=PROJECT)
...
QUERY_BASE = {"session": [project_id], "filter": "eq(is_root, true)"}
...
for _ in range(3):
    while True:
        ...
        rr = S.post(API + "/runs/query", json={**QUERY_BASE, "limit": 25}, timeout=30)
...
roots = rr.json().get("runs", [])
metrics["notes"]["recent_root_run_names"] = sorted({x.get("name") or "<none>" for x in roots})
...
rt = post(
    ...
    {
        "session": [project_id],
        "filter": 'and(eq(run_type, "tool"), eq(name, "get_weather"))',
        "limit": 10,
    },
)
tool_runs = rt.json().get("runs", []) if rt.status_code == 200 else []
if not tool_runs:
    rt = post(
        "/runs/query",
        {"session": [project_id], "filter": 'eq(run_type, "tool")', "limit": 10},
    )
    tool_runs = rt.json().get("runs", []) if rt.status_code == 200 else []
if not tool_runs:
    ...
    ra = post("/runs/query", {"session": [project_id], "trace": trace_id, "limit": 100})
    trace_runs = ra.json().get("runs", [])
    llm_runs = [x for x in trace_runs if x.get("run_type") == "llm"]
    t_runs = [x for x in trace_runs if x.get("run_type") == "tool"]

    # fetch one llm run and one tool run in full detail
    g = get("/runs/" + llm_runs[0]["id"]).json() if llm_runs else {}
    t = get("/runs/" + t_runs[0]["id"]).json() if t_runs else {}

    extra = g.get("extra") or {}
    model = (
        (extra.get("metadata") or {}).get("ls_model_name")
    ...
    tree_ok = any(x.get("parent_run_id") for x in trace_runs)
    comp = {
        "trace_id": trace_id,
        "root_run_name": next(
            (x.get("name") for x in trace_runs if not x.get("parent_run_id")), None
        ),
...
rtm = post(
    ...
    {
        "session": [project_id],
        "filter": f'and(eq(is_root, true), gt(start_time, "{since}"))',
        "limit": 5,
    },
)
qf["filter_by_time"] = rtm.status_code == 200 and "runs" in rtm.json()

rn = post(
    "/runs/query",
    {"session": [project_id], "filter": 'eq(name, "chat-hello")', "limit": 5},
)
rn_runs = rn.json().get("runs", []) if rn.status_code == 200 else []
...
# server-side aggregation: /runs/stats returns counts/latency/token aggregates
rs = post("/runs/stats", {"session": [project_id], "filter": "eq(is_root, true)"})
...
metrics["notes"]["runs_stats_sample"] = (
    {k: stats.get(k) for k in ("run_count", "total_tokens", "median_tokens", "total_cost")}
...
rsql = post(
    "/runs/query",
    {"session": [project_id], "filter": "select * from runs", "limit": 1},
)
qf["free_sql_or_dsl"] = False
metrics["notes"]["free_expression_probe"] = f"HTTP {rsql.status_code}: {rsql.text[:200]}"
metrics["query_flexibility"] = qf

# ---------- 5. pagination ----------
pag = {"mechanism": None, "second_page_fetched": False}
p1r = post("/runs/query", {**QUERY_BASE, "limit": 2})
p1 = p1r.json()
cursors = p1.get("cursors") or {}
pag["first_page_cursor_keys"] = sorted(cursors.keys())
nxt = cursors.get("next")
pag["mechanism"] = "cursor (cursors.next token in POST /runs/query response)" if "next" in cursors else "unknown"
if nxt:
    p2r = post("/runs/query", {**QUERY_BASE, "limit": 2, "cursor": nxt})
    p2_runs = p2r.json().get("runs", []) if p2r.status_code == 200 else []
    ids1 = {x["id"] for x in p1.get("runs", [])}
    pag["second_page_fetched"] = bool(p2_runs) and p2_runs[0]["id"] not in ids1
...
if (p1r.headers.get("content-type") or "").startswith("application/json"):
    fmts.append("json")
rc = post("/runs/query", {**QUERY_BASE, "limit": 1}, headers={"Accept": "text/csv"})
...
try:
    ...
    if proc.returncode != 0:
        ...
        deadline = t_emit + 120
        flt = f'and(eq(name, "chat-hello"), gt(start_time, "{frm}"))'
        while time.perf_counter() < deadline:
            rr = post(
                "/runs/query",
                {"session": [project_id], "filter": flt, "limit": 5},
            )
            if rr.status_code == 200 and rr.json().get("runs"):
...
bad = post("/runs/query", {"session": [project_id], "filter": "eq(", "limit": 1})
...
metrics["dx_friction"] = (
    ...
    "to a UUID via GET /sessions?name=. Listing runs is a POST /runs/query with a "
    "small function-style filter DSL (eq/and/or/gt/has/search) -- expressive for "
    "filtering but no server-side arbitrary expressions; aggregates come from a "
    "separate fixed-shape /runs/stats endpoint. Cursor pagination via cursors.next. "
    "Rate limit of 10 req/10s forces client-side throttling in any script. "
    f"Malformed filter 'eq(' returned HTTP {bad.status_code} "
    + ("with a structured error body." if bad.status_code in (400, 422) else "-- see notes.malformed_query_response.")
)
