...
Latest results (August 15, 2026 — Braintrust's eval moved to standard SQL over `/btql` with real multi-format exports via community PR #7, so all five read APIs were measured back-to-back in the same session, continuous scoring):

| Rank | Platform | Score / 100 | What decided it |
|------|----------|------------:|-----------------|
| 🥇 | [Phoenix](./phoenix) (Arize) | **87.27** | Fastest read path (93.5 ms) and write-to-read lag (0.1 s), verbatim `gen_ai.*` + OTLP-JSON export; weakest query surface of the top three (no server-side aggregation or SQL) |
| 🥈 | [Braintrust](./braintrust) | 86.20 | **Biggest mover (+5.03)**: standard SQL over `project_logs()` and three real export formats via `fmt` — json, ndjson, **parquet** — plus 0.4 s write-to-read |
| 🥉 | [LangSmith](./langsmith) | 83.54 | 98.8 ms retrieval and 0.1 s freshness on the v2 query APIs; no free query language, fixed-shape stats only |
| 4 | [Logfire](./logfire) (Pydantic) | 82.94 | Arbitrary SQL, verbatim `gen_ai.*`, 4 export formats — but this cohort's worst measured latency/freshness, which cohort normalization compresses to 0; still dominates the 10k-corpus query bench |
| 5 | [Langfuse](./langfuse) | 77.42 | 165 ms retrieval, cursor pagination; ingest lag back to 8.6 s after Aug 13's 43.5 s outlier; JSON-only export |
...
- **How you call the API is a measurement variable as big as vendor performance.** Langfuse jumped after its observations v2 API (1293 → ~140-240 ms); LangSmith after leaving the deprecated v1 query path (1935 → ~100-125 ms); Braintrust after its own team pointed the eval at SQL-over-/btql with `fmt` exports (export-formats 5 → 10). Same platforms, same data — different endpoints.
...
| Braintrust | `POST api.braintrust.dev/btql` (+ legacy `/fetch`) | Bearer API key | SQL over `project_logs()` (BTQL pipe still accepted) | JSON, NDJSON, Parquet |
...
- **Braintrust** `/btql` takes a `fmt` parameter (`json`/`jsonl`/`csv`/`parquet`) — csv returns 400, the rest deliver; legacy `/fetch` pagination can re-return rows across pages — dedupe by `id`. Demo traces age out of projects (observed between Aug 3 and Aug 12) — repopulate before measuring completeness.
