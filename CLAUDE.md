...
- July raw evidence (`results/{platform}.json`) is frozen too. Re-runs go to new files: `results/*_aug2026.json` + `results/langfuse_v2.json` (Aug 3 cohort, `--cohort aug2026`); `results/*_aug12_2026.json` + `results/phoenix.json` (Aug 12 five-platform cohort with Phoenix, `--cohort aug12_2026`; Phoenix categorical scores from `results/phoenix_judges.json`). If a vendor ships improvements, re-run **all five** evals back-to-back — rubric rule 2 forbids mixing sessions in one cohort. Eval scripts honor `EVAL_RESULTS_PATH` to redirect output without touching frozen files.
