...
- **Braintrust** `POST /btql` (SQL over `project_logs('<id>')`; legacy pipe syntax still accepted) + legacy `GET /v1/project_logs/{id}/fetch`: Bearer auth. `/btql` takes `fmt` — json/jsonl/parquet deliver real payloads (parquet = PAR1 magic), csv → 400. Fetch pagination can re-return rows — dedupe by `id`; `/fetch` has **no server-side time filter** — separate coexisting corpora client-side via `metrics.start`. Resource attributes are NOT visible in fetch rows — isolation must be by project. **Demo traces age out of projects** (July demos vanished between the Aug 3 and Aug 12 cohorts) — repopulate demos before measuring completeness.
...
- July raw evidence (`results/{platform}.json`) is frozen too. Re-runs go to new files: `results/*_aug2026.json` + `results/langfuse_v2.json` (Aug 3 cohort); `results/*_aug12_2026.json` + `results/phoenix.json` (Aug 12, Phoenix joins; its categorical scores from `results/phoenix_judges.json`, a fresh 3-judge panel); `results/*_aug13_2026.json` (Aug 13, LangSmith v2 migration); `results/*_aug15_2026.json` (Aug 15, Braintrust SQL/fmt-exports migration via PRs #7/#12). If anything changes measurement conditions (vendor improvement, endpoint migration, new platform), re-run **all five** evals back-to-back — rubric rule 2 forbids mixing sessions in one cohort.
- Categorical criteria keep judge averages across cohorts; verifiable API changes get **mechanical rubric-anchor overrides** declared in the cohort config (langfuse v2: pagination 10 / otel-fidelity 9 / query-flex 8; langsmith v2: dx-friction 7; braintrust SQL: export-formats 10) — flagged as provisional, never silently.
