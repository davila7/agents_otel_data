...
An AI-agent observability benchmark: the same three traced scenarios are written into five OTEL-based platforms (Logfire, Braintrust, LangSmith, Langfuse, Arize Phoenix), then each platform's **read API** is benchmarked with executable scripts and scored by a judge panel. `README.md` has the verdict; `evaluation/RESULTS.md` has the full methodology and caveats.
...
- July raw evidence (`results/{platform}.json`) is frozen too. Re-runs go to new files: `results/*_aug2026.json` + `results/langfuse_v2.json` (Aug 3 cohort, `--cohort aug2026`); `results/*_aug12_2026.json` + `results/phoenix.json` (Aug 12 five-platform cohort with Phoenix, `--cohort aug12_2026`; Phoenix categorical scores from `results/phoenix_judges.json`); `results/*_aug13_2026.json` (Aug 13 cohort after LangSmith's eval moved to the v2 query APIs, `--cohort aug13_2026`). Since Aug 13 the Logfire/Langfuse projects also hold the `dataset/` synthetic corpus — their evals carry exclusion filters (`SYNTH_EXCL` SQL predicate / fetch-demo-trace-by-name) that must be preserved. If a vendor ships improvements, re-run **all five** evals back-to-back — rubric rule 2 forbids mixing sessions in one cohort. Eval scripts honor `EVAL_RESULTS_PATH` to redirect output without touching frozen files.
